                ...
                <div class="content-grid">
                    <table class="table-grid-load">
                        ...
                        <tbody>
                            ...
                            <tr v-for = "(item, index) in users" :key="item.userId" :ref = "item.userId" @mouseover = "showFeatureRecord(item.userId)" @mouseleave = "hideFeatureRecord(item.userId)">
                                <th class="checkbox-table">
                                    <input type = "hidden" class = "input-hidden" />
                                    <input type="checkbox" class = "grid-checkbox" v-on:click = "btnSeclectedDelete(item.userId,$event)"> 
                                </th>
...
export default {
    ...
    methods : {
         ...
         * createdBy : DNSon
        */
        btnSeclectedDelete : function(userId , event)
        {
            try {
                var self = this;
                this.isClickCheckbox = true;
                var checkbox =  event.target;
                var trSelected = self.$refs[userId];
                if(checkbox.checked)
                {
                    trSelected[0].classList.add("selected-record");
                    this.listIdSelectedRecord.push(userId);
                }else{
                    trSelected[0].classList.remove('selected-record')
                    var newArray = CommonFunction.arrayRemove(this.listIdSelectedRecord , userId);
                    this.listIdSelectedRecord = newArray;
                }
                self.$emit("insertuserIdToListId" , this.listIdSelectedRecord);
         ...
         * CreatedBy : DNSon (19/11/2021)
        */
        hideFeatureRecord : function(userId)
        {
            try {
                var me = this;
                var trSelected = me.$refs[userId];
